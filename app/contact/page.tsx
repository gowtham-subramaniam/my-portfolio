...
export default function Contact() {
  ...
  />
        ...
        <div style={{ display: "flex", gap: "0.15rem" }}>
          {[["About", "/"], ["Resume", "/resume"], ["Blog", "/blog"], ["Contact", "/contact"]].map(([label, href]) => (
            <Link key={label} href={href} style={{
              fontSize: 15,
              fontWeight: href === "/contact" ? 700 : 500,
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        {status === "success" && (
          <div style={{ fontSize: 15, color: "#1a5c2a", fontWeight: 500, background: "#f0faf0", padding: "16px 20px", borderRadius: 8, border: "1px solid #c0e8c0", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 20 }}>✓</span>
            Message sent! I&apos;ll get back to you soon.
          </div>
        )}
        {status === "error" && (
          <div style={{ fontSize: 15, color: "#8b1a1a", fontWeight: 500, background: "#fdf0f0", padding: "16px 20px", borderRadius: 8, border: "1px solid #e8c0c0", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 20 }}>✗</span>
        ...
        <div>
          ...
          <input name="subject" value={form.subject} onChange={handleChange} required placeholder="Collaboration opportunity" style={inputStyle} />
        </div>

        <div>
          <label style={labelStyle}>Message *</label>
          <textarea name="message" value={form.message} onChange={handleChange} required placeholder="Tell me about your project or opportunity..." rows={6} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.7 }} />
        </div>

        <button type="submit" disabled={status === "sending"} style={{
          background: status === "sending" ? "var(--subtle)" : "var(--btn-bg)",
          color: "var(--btn-text)", padding: "12px 28px", borderRadius: 8,
      ...
      <footer style={{ marginTop: "4rem", paddingTop: "1.5rem", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
        ...
        <div style={{ display: "flex", gap: "1.5rem", alignItems: "center" }}>
          ...
          <a href="https://www.linkedin.com/in/gowthamsubramaniam/" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--subtle)", textDecoration: "none" }}>
